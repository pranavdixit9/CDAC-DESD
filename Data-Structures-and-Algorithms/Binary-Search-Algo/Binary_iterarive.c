...
int main()
{
	...
	int f = binSearch(arr, 0, size, sear);
	if(f == -1)
	{
...
int binSearch(int *arr, int start, int end, int sear) {
    while (start <= end) 
    {
        ...
        if (arr[mid] < sear) 
        {
            start = mid + 1;						//If the middle element is less than the target, update the 'start' to search the right half.
        } else 
        {
            end = mid - 1;      			    	// If the middle element is greater than the target, update the 'end' to search the left half.
        }
    }
